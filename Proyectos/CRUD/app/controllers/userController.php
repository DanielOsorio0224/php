<?php
    ...
    class userController extends mainModel{
        ...
        public function listrarUsuarioControlador($pagina,$registros,$url,$busqueda){
            ...
            if(isset($busqueda) && $busqueda !=""){
                $consulta_datos = "SELECT * FROM usuario WHERE ((usuario_id !='".$_SESSION['id']."' AND usuario_id !='1') 
                AND (usuario_nombre LIKE '%$busqueda%' OR usuario_apellido LIKE '%$busqueda%' OR usuario_email LIKE '%$busqueda%'
            ...
            if($total>=1 && $pagina<=$numeroPaginas){
                ...
                foreach($datos as $rows){
                    $tabla.='
                        ...
                        <td><a href="'.APP_URL.'userPhoto/'.$rows['usuario_id'].'/" class="button is-info is-rounded is-small">Foto</a></td>
                        <td><a href="'.APP_URL.'userUpdate/'.$rows['usuario_id'].'/" class="button is-success is-rounded is-small">Actualizar</a></td>
                        <td>
                            <form action="'.APP_URL.'app/ajax/usuarioAjax.php" class="FormularioAjax" method="POST" autocomplete="off">
        ...
        public function actualizarUsuarioControlador(){
                ...
                if($check_admin['usuario_usuario']!=$admin_usuario ||
                 !password_verify($admin_clave,$check_admin['usuario_clave'])){
                    $alerta=[
                        "tipo"=>"simple",
                        "titulo"=>"Ocurrio un error",
                        "texto"=>"No coincide el usuario o la contraseña",
                        "icono"=>"error"
                    ];
                    return json_encode($alerta);
                    exit();    
            ...
            if($this->actualizarDatos("usuario",$usuario_datos_up,$condicion)){
                ...
                $alerta=[
                    ...
                    .$datos['usuario_apellido']." fue actualizado correctamente.",
                    "icono"=>"success"
